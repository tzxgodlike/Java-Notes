...
public class StackPushPopOrder {
     ...
     * （注意：这两个序列的长度是相等的）
     */
    public static boolean isPopOrder(String push,String pop) {
        if (push == null || pop == null || push.length() == 0 || pop.length() == 0) {
            return false;
        }
        char[] pushchar = push.toCharArray();
        char[] popchar = push.toCharArray();
        Stack<Character> stack = new Stack<>();

        //出栈序列
        int popindex = 0;

        for (int i = 0;i<pushchar.length;i++) {
            //先入栈
            stack.push(pushchar[i]);
            // 每入栈一次和出栈序列比较，如果栈顶和当前出栈元素相同，则弹出同时当前弹出元素指针前移；
            // 如果下一个栈顶元素还和当前弹出元素相同，继续弹出
            while (!stack.isEmpty()&&stack.peek()==popchar[popindex]) {
                stack.pop();
                popindex++;
            }
        }
